...
class EditorViewController: UIViewController, EDHFinderListViewControllerDelegate, MFMailComposeViewControllerDelegate {
    ...
    lazy var fullscreenItem: UIBarButtonItem = {
        let item = UIBarButtonItem(image: nil, style: .plain, target: self,
                        action: #selector(fullscreenItemDidTap))
        return item
    }()
    
    lazy var reloadItem: UIBarButtonItem = {
        let item = Utility.barButtonItem(target: self,
    ...
    lazy var countItem: UIBarButtonItem = {
    ...
    lazy var editorView: EditorView = {
        let eView = EditorView(frame: self.view.bounds)
        self.view.addSubview(eView)

        eView.onChangeText = updateCountItem
        return eView
    }()
    ...
    @objc func shareItemDidTap(_ sender: AnyObject) {
        ...
        alertController.addAction(UIAlertAction(
            ...
            handler: { (_: UIAlertAction!) in
                ...
                var items: [String] = [item.fileURL()?.absoluteString ?? ""]
